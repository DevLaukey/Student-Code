...
void Cpu::thread_ex_start()
{
    ...
    if (wb_regWrite && wb_registerNum != 0)
    {
        ...
        {
            forwardA = 1;
            printf("Forwarding RS from WB to ALU input\n");
        }
        if (wb_registerNum == rtidx && forwardB == 0)
        {
            forwardB = 1;
            printf("Forwarding RT from WB to ALU input\n");
        }
    }
...
bool Cpu::isProgramComplete() const
{
    ...
    return (regIFID_IDside.instruction == 0x08000062); // Assuming 0x88 is the address of "done:" instruction
}
